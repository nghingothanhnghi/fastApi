# app/camera_object_detection/routes/object_detection_router.py
...
#     await websocket.accept()   # ✅ accept once at start
...
#                     results["hardware_stats"] = HardwareDetectionService.get_hardware_statistics(enhanced_detections)
...
#                 logger.info("Object detection WebSocket disconnected by client")
#                 break

#             except Exception as e:
#                 logger.error(f"Error processing WebSocket message: {e}")
#                 await websocket.send_json({"error": f"Processing error: {str(e)}"})
#                 # Optional: break if unrecoverable
#                 break
...
#         try:
#             if detector:
#                 # Add any detector cleanup logic here if needed
#                 pass
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    A websocket connection can stay open far longer than a normal HTTP
    request (minutes to hours). A dependency-injected session would be
    checked out of the connection pool for that whole time, not just while
    a query runs — a handful of open dashboard tabs is enough to exhaust
    the pool (size=5, overflow=10) and start timing out unrelated requests
    elsewhere in the app (e.g. `QueuePool limit ... reached` errors from
    the background jobs). Instead, a short-lived SessionLocal() is opened
    and closed right around the one per-frame DB lookup that needs it.
    """
    await websocket.accept()   # ✅ accept once at start
    detector = ObjectDetector()
    logger.info("Object detection WebSocket connection established")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    # Short-lived session: opened just for this lookup and
                    # closed immediately after, instead of being held for
